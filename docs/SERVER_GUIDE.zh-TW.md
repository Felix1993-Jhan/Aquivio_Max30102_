...
> **目前版本 `0.0.0.6`** — 用 `./max30102_server --version` 或 `/health` 的 `version` 欄位確認手上是哪一版。
...
## ⚠️ 已知硬體問題：有些 MAX30102 模組把兩顆 LED 裝反了

> 2026-09-08 查證。**這一節是給採購與現場除錯看的**，服務本身已經會自動處理。

### 症狀

血氧永遠是 `null`，但心率、波形、HRV 全部正常。

### 原因

MAX30102 的 datasheet 規定：`0x0C` = LED1 = **紅光**、`0x0D` = LED2 = **紅外**，
FIFO 每組 6 bytes 是 **RED 在前、IR 在後**。我們的解碼完全照規格。

但實測手上 8 片模組，其中 **7 片把兩顆 LED 晶粒裝反了** —— 送出來的兩路是對調的。
（今華電子 70 元那批全部有問題；廣鏵 250~300 元那片正常。）

兩路標反之後，血氧的 R 比值變成倒數，算出來是生理上不可能的值
（實測 R = 2.35 → 血氧 −83%），被合法範圍檢查擋掉 → `spo2` 回 `null`。

### 30 秒判別法（零設備）

```
① 40 71 31 09 01 0D 00 00 07     關掉 0x0D（datasheet 說那是紅外）
② 用眼睛看感測器
③ 40 71 31 09 04 00 00 00 11     RE-INIT 復原
```

| 第 ② 步看到 | 結論 |
|---|---|
| **紅光消失** | **這片裝反了** —— `0x0D` 實際驅動的是紅光晶粒 |
| 紅光還在 | 正常，符合 datasheet |

660nm 紅光肉眼清楚可見；880nm 紅外肉眼完全看不到（手機相機拍得到淡紫白色光點）。

### ⚠️ 不能靠讀數大小判斷

「哪一路的數值比較大」**完全沒有識別資訊**。實測：

| 板子 | 第 1 槽 | 第 2 槽 | 誰大 | 真正的 IR 是 |
|---|---|---|---|---|
| 廣鏵（正常） | 80,411 | 68,567 | 第 1 槽 | **第 2 槽** |
| 今華（裝反） | 142,750 | 133,603 | 第 1 槽 | **第 1 槽** |

**同樣的大小順序，相反的真相。** 因為 DC 讀值取決於 LED 亮度、光路耦合與
光電二極體響應 —— 那是硬體差異，每片都不同。有識別資訊的只有 **AC/DC 比值**
（除以 DC 正好把硬體差異消掉）。

### 服務怎麼處理

核心**每次量測自己判定一次**（`K2ChannelOrient`），靠的就是 AC/DC 比值：

```
R < 0.9   → 目前假設正確
R > 1.3   → 目前假設相反
0.9~1.3   → 分不出來 → 不判定、不給血氧（心率照給）
```

判定完成前**不輸出波形樣本** —— 與沉澱期同一個原則：寧可晚兩秒，也不要送出
貼錯標籤的資料。所以第一筆波形比 `0.0.0.5` 晚約 2 秒。

判定結果在 `/vitals` 的 `channelOrient` 欄位：

| 值 | 意思 |
|---|---|
| `unknown` | 還在判定（此時沒有波形、沒有血氧，心率照給） |
| `normal` | 符合 datasheet |
| `swapped` | 這片裝反了 —— **輸出已經由服務轉正**，你拿到的 `ir` / `red` 是實際的光源 |

方向掛在「這一次量測」上，手指離開就歸零，下一位重新判定 —— 所以中途換板子、
或同一台機器接不同模組，都會自動處理，不需要任何設定。

### 給採購的建議

70 元的整片模組**低於原廠晶片單顆的成本**，那顆不可能是原廠件。
血氧若只是內部測試，配合上面的目視檢驗就夠；若要出現在給客戶看的畫面上，
建議回頭走授權代理 —— 因為**波長偏差是安靜的錯**，數字看起來正常但系統性偏掉，
你不會發現，直到拿真的血氧機對照。

（目前對調後的血氧與正常板差不到 0.4%，看不出波長問題，但那是在接近 100%
的區間量的，曲線很平，不足以當作驗證。）
