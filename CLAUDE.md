...
### 通道方向自動判定（`K2ChannelOrient`）

**實測 8 片 MAX30102 模組，7 片把兩顆 LED 晶粒裝反**（今華 70 元那批全中；廣鏵 250~300 元那片正常）。datasheet 規定 `0x0C` = LED1 = 紅光、FIFO 順序 RED 在前，我們的解碼完全照規格 —— 錯的是模組。兩路標反之後血氧的 R 變成倒數，算出來是生理上不可能的值（R = 2.35 → 血氧 −83%），被範圍檢查擋掉變成 `null`。

核心因此**每次量測自己判定一次方向**。四個容易踩的點：

1. **不能靠讀數大小判斷。** DC 高低只反映「LED 多亮、光路耦合多好」，每片都不同。實測正常板與對調板的 DC 大小順序**一樣**，真相卻相反 —— 同一個特徵對應兩個相反答案，等於沒有訊號。有識別資訊的只有 **AC/DC 比值**（= 血氧的 R），除以 DC 正好把硬體差異消掉。
2. **方向只在兩個地方套用** —— `_compute()` 呼叫演算法時、以及組 `K2FeedResult` 時。**緩衝區永遠存線上收到的原始槽位，不做交換。** 這樣下游（波形圖、快照、`/waveform`、WS `wave`）全部自動拿到實際光源，不會有「某處換了、某處忘了」的漂移。
3. **判定完成前不輸出樣本**，與沉澱期同一個原則：寧可晚兩秒，也不要送出貼錯標籤的資料。鎖定那一刻把壓著的一次吐出（沿用沉澱期「200 筆一次吐」的做法）。有 `orientTimeoutMs` 逃生口，避免訊號差的人連波形都看不到。
4. **模糊帶（R 在 0.9~1.3）刻意不判定。** 那裡對應血氧 55~87%，「真的缺氧」與「通道接反」分不開。分不出來就不猜 —— **不能把危險值自動改寫成正常值**。血氧另有 `_orientByVote` 閘門：逾時退回預設方向時方向沒被驗證過，照樣不給血氧。

方向掛在**這一次量測**上，手指離開就跟著 `_totalSamples` 一起歸零（`_clearOrient()`）。新增清除點時別漏掉，漏了會把上一位的判定套到下一位。

判別模組是不是裝反的（30 秒、零設備）：送 `40 71 31 09 01 0D 00 00 07` 關掉 `0x0D`（datasheet 說那是紅外），**紅光消失就是裝反的**。完整查證過程在 [SERVER_GUIDE.zh-TW.md](docs/SERVER_GUIDE.zh-TW.md) 最下方。
...
⚠️⚠️ **兩塊板二選一在線，所以「送」的那一側不可以寫死站號。** 收的那一側（`_parseUrReadResponse`）本來就兩個都認，但心跳 PING 與自動連線驗證原本都寫死 `0x30` —— 擴充板單獨在線時，手動送 `0x31` 明明收得到回應，心跳卻連續落空、三次之後誤報「連接可能已斷開」，而且**心跳送出不寫日誌**，日誌看起來就是「剛連上就莫名其妙斷了」。

現在兩處都改成 `0x30 ↔ 0x31` 交叉輪替（`SerialPortManager._pingBoards`）。輪替同時解掉誤判：擴充板在線時 `0x30` 那次落空（計數 +1）、下一次 `0x31` 收到回應就歸零，計數在 0↔1 擺盪永遠到不了門檻；真的兩塊都沒回應才會累加到觸發。

這個問題以前沒浮出來，是因為 **App 開機首頁就是 K2 頁，而切進 K2 頁會 `stopHeartbeat()`** —— 只有停在 UR 面板那一頁才看得到。
