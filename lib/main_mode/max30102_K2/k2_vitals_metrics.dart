...
class HrvSpectrum {
  ...
  // ── 可用性門檻:兩者都是「頻帶低頻邊緣約走滿 4 個週期」──────────────
  //
  // 一個頻帶要估得準,至少得在它**最低的那個頻率**上看到約 4 個完整週期。
  // 兩個門檻是同一條規則套在不同的頻帶下緣:
  //
  //   HF 下緣 0.15Hz → 週期 6.7 秒 → 4 圈需要  27 秒 → 門檻取 25 秒(3.75 圈)
  //   LF 下緣 0.04Hz → 週期  25 秒 → 4 圈需要 100 秒 → 門檻取 110 秒(4.4 圈)
  //
  // ⚠️ 兩個門檻都**刻意訂得比整數秒數低一點**,因為一段「錄滿 N 秒」的資料
  //    實際跨度必然略小於 N —— 第一拍的起谷落在 t=0 之後、最後一拍的終谷落在
  //    t=N 之前,兩端各少一點。門檻若寫死 N 就會把它本來要接受的那種錄製
  //    整批擋掉(實測:2 分鐘錄製跨度 119.x、30 秒視窗跨度 27~29)。
  ...
  /// **HF 這個數字能不能用。** 門檻寬鬆得多,產品的 30 秒視窗就過得了 ——
  /// 這正是「LF/HF 裡只有 HF 那一半可信」的來源。
  bool get hfUsable => spanSeconds >= 25;
}
